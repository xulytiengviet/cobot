...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{window.__videoDraws=0;window.__overlayDraws=0;window.__popupDraws=0;window.__cameraCalls=0;const gum=navigator.mediaDevices.getUserMedia.bind(navigator.mediaDevices);navigator.mediaDevices.getUserMedia=(...args)=>{window.__cameraCalls++;return gum(...args);};const draw=CanvasRenderingContext2D.prototype.drawImage;CanvasRenderingContext2D.prototype.drawImage=function(source,...args){if(this.canvas.id==='overlay'){window.__overlayDraws++;if(source instanceof HTMLVideoElement)window.__videoDraws++;}if(this.canvas.id==='popupCanvas'&&source instanceof HTMLVideoElement)window.__popupDraws++;return draw.call(this,source,...args);};});
  await page.goto('http://localhost:8000/');
  await page.waitForFunction(()=>document.querySelector('#modelStatus').textContent.includes('SẴN SÀNG'));
  await page.getByRole('button',{name:'Bật camera',exact:true}).click();
  ...
  const overlayBefore=await page.evaluate(()=>window.__overlayDraws);
  await page.locator('#visibility').selectOption('full');
  await page.waitForFunction(n=>window.__overlayDraws>n,overlayBefore);
  assert.equal(await page.evaluate(()=>window.__videoDraws),0,'Accepted AI frame canvas is used instead of a newer live video frame');
  ...
  assert(mobile.width<=220&&mobile.x>=0&&mobile.x+mobile.width<=390);
  assert(mobile.y>=0&&mobile.y+mobile.height<=844);
  await page.evaluate(()=>window.scrollTo(0,0));
  assert(await page.locator('#popupCalibrate').evaluate(el=>{const r=el.getBoundingClientRect();const hit=document.elementFromPoint(r.x+r.width/2,r.y+r.height/2);return hit===el||el.contains(hit);}), 'Floating calibration remains visible after scrolling');
  ...
  assert(await page.locator('#videoPopup').evaluate(el=>el.classList.contains('docked')),'Mobile video must dock into document flow');
  assert(await page.locator('#videoPopup').evaluate(el=>el.parentElement?.id==='mobileVideoSlot'),'Mobile dock must use the reserved slot');
  const compactHeight=(await page.locator('#videoPopup').boundingBox()).height;
  await page.locator('#popupSize').click();
  assert((await page.locator('#videoPopup').boundingBox()).height>compactHeight,'Expand changes height without floating over controls');
  await page.locator('#popupSize').click();
  assert(await page.locator('#videoPopup').isVisible(),'Minimize must not hide video');
  assert((await page.locator('#videoPopup').boundingBox()).width<=390);

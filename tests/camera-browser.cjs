...
(async()=>{
 ...
 try{
  ...
  await page.addInitScript(()=>{window.__videoDraws=0;const draw=CanvasRenderingContext2D.prototype.drawImage;CanvasRenderingContext2D.prototype.drawImage=function(source,...args){if(this.canvas.id==='overlay'&&source instanceof HTMLVideoElement)window.__videoDraws++;return draw.call(this,source,...args);};});
  await page.goto('http://localhost:8000/');
  await page.waitForFunction(()=>document.querySelector('#modelStatus').textContent.includes('SẴN SÀNG'));
  await page.getByRole('button',{name:'Bật camera',exact:true}).click();
